...
void flip_image(const cv::Mat& image) {
...
void Engine::run() {
    ...
    if (cap_.isOpened()) {
        ...
        while (1) {
            ...
            if (display_image() == 27) { break; }
            // save_frame();
        }
    }
}

 Engine::Engine(const std::uint32_t& num_bubbles)  :
 cap_(0)
//  (const int num_bubbles) 
    {
        if(!cap_.isOpened()) {
            std::cout << "Camera initialization failed." << std::endl;
        }  else {
            get_frame();
            initialize_bubbles(num_bubbles);
        }
    }
...
void Engine::initialize_bubbles(const std::uint32_t& num_bubbles) {
    ...
    std::uniform_int_distribution<> distr_pos_x(PADDING, frame_bgr_.cols - PADDING);
    std::uniform_int_distribution<> distr_pos_y(PADDING, frame_bgr_.rows - PADDING); 
    std::uniform_int_distribution<> distr_vec(-10, 10); // define the range for velocities

    // assign initial values
    for (int i = 0; i < num_bubbles; i++) {
        bubbles_.emplace_back(
            static_cast<float>(distr_pos_x(gen)),
